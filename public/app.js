...
// ================= CONFIG — the only lines you should edit =================
const ADSGRAM_BLOCK_ID = 'YOUR_ADSGRAM_BLOCK_ID'; // ←←← PASTE YOUR NEW ADSGRAM BLOCK ID HERE
const ADSGRAM_DEBUG = true; // true = TEST ads while checking. Set to false once ads work → real ads + real earnings
const SPONSOR_AD_FN = 'show_11648455'; // Monetag rewarded interstitial function name
const PROOF_CHANNEL = 'https://t.me/elzuno_officials'; // your public payouts channel
// ============================================================================
...
function showRewardedAd() {
  return new Promise((resolve) => {
    if (!_adsgram) {
      // SDK didn't load or no block id — mock so the flow still works during testing
...
// ---------- Payouts proof (public feed — fills Home + Cash tab) ----------
function loadPayouts() {
  ...
  api('/api/payouts').then((data) => {
    ...
    fill($('homePayoutsList'), 3);  // top 3 on the Home screen
    fill($('payoutsList'), 12);     // full list on the Cash tab
  }).catch(() => {});
}
...
function demoMode() {
  ...
  demoLeaderboard();
  if ($('homeProofChannel')) $('homeProofChannel').href = PROOF_CHANNEL;
  if ($('homePayoutsList')) $('homePayoutsList').innerHTML = '<div class="wd-item"><div><b>Adaeze O.</b><br><span class="muted" style="margin:0">Today</span></div><span class="badge paid">₦1,000 ✓</span></div><div class="wd-item"><div><b>Tunde A.</b><br><span class="muted" style="margin:0">Yesterday</span></div><span class="badge paid">₦2,500 ✓</span></div>';
...
// ---------- Actions: sponsor ad (Monetag rewarded interstitial) ----------
$('sponsorBtn').addEventListener('click', async () => {
  $('sponsorBtn').disabled = true; $('sponsorStatus').textContent = 'Loading ad…';
  if (DEMO) { hap('success'); S.balance += 30; S.naira = S.balance / 100; renderBalance(); $('sponsorStatus').textContent = '+30 pts! 🎉'; setTimeout(() => { $('sponsorBtn').disabled = false; }, 800); return; }
  const fn = window[SPONSOR_AD_FN];
  if (typeof fn !== 'function') { $('sponsorStatus').textContent = '⚠️ Sponsor ad not loaded — paste Monetag script tag in index.html <head>'; $('sponsorBtn').disabled = false; return; }
  // fn() resolves after the user watches the ad; race with a 45s safety timeout so the button never hangs
  const watched = await Promise.race([
    Promise.resolve(fn()).then(() => true).catch(() => false),
    new Promise((r) => setTimeout(() => r(false), 45000)),
  ]);
  if (!watched) { $('sponsorStatus').textContent = 'Ad closed or unavailable.'; $('sponsorBtn').disabled = false; return; }
  const data = await api('/api/sponsor', { initData: initData() });
  if (data.ok) { hap('success'); S.balance = data.balance; S.naira = data.balance/(S.points_per_naira||100); renderBalance(); $('sponsorStatus').textContent = `+${data.gained} pts! 🎉`; }
  else if (data.error === 'cooldown') $('sponsorStatus').textContent = 'Sponsor ready in ' + fmtCountdown(data.retry_in_ms);
  else $('sponsorStatus').textContent = 'Try again later.';
